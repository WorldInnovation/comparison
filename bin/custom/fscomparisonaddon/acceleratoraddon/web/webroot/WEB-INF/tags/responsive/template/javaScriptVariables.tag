...
<%-- JS configuration --%>
	<script type="text/javascript">
		...
		<%-- THIS WILL BE CACHED AS PART OF THE DOM DOCUMENT: DO NOT ASSIGN SESSION-DEPENDENT VARIABLES --%>
		var ACC = { config: {} };
        	...
        	ACC.addToCartMessage = '<spring:theme code="pdp.select.size.button"/>';
        	ACC.deactivateConfirmMessage = '<spring:theme code="text.account.profile.deactivate.confirm"/>';
			ACC.config.googleApiKey="${googleApiKey}";
			ACC.config.googleApiVersion="${googleApiVersion}";
			ACC.storelocatorDefaultLatitude = '<spring:eval expression="@siteConfigService.getProperty('storefront.storelocator.latitude')" />';
		    ACC.storelocatorDefaultLongitude = '<spring:eval  expression="@siteConfigService.getProperty('storefront.storelocator.longitude')" />';
			ACC.reviewsDataUrl = '<spring:eval expression="@siteConfigService.getProperty('product.verified.reviews.data.url')" />';
			ACC.reviewsStarsUrl = '<spring:eval expression="@siteConfigService.getProperty('product.verified.reviews.stars.url')" />';

			<spring:eval expression="T(es.fs.core.store.util.MultiCountryUtil).extractBusinessFromCmsSite(@cmsSiteService.getCurrentSite())" var="business"/>
			ACC.config.business="${business}";
		...
		//ACR FS-2119
		ACC.config.payin7Authorization = '<spring:eval expression="@siteConfigService.getProperty('fsstorefront.payin7.authorization')" />';
		ACC.config.payin7Url = '<spring:eval expression="@siteConfigService.getProperty('fsstorefront.payin7.url')" />';
		ACC.config.payin7ContentType = '<spring:eval expression="@siteConfigService.getProperty('fsstorefront.payin7.contentType')" />';
